...
%---------------------------------------------------------------------
% yang, add here for aSymmetric echo downsampling
properties(GetAccess='public', SetAccess='public')
    NCol_protocol  % the information from protocol
end
%---------------------------------------------------------------------
...
%         end
    ...
    function varargout = subsref(this, S)
        ...
        if this.arg.removeOS
            if this.NCol<this.NCol_protocol*2
                outSize(1)=this.NCol_protocol;
            end
        end
        %---------------------------------------------------------------------
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        %out = reshape(out, selRangeSz(1), selRangeSz(2),[]);%yang:original
        out = reshape(out, outSize(1), selRangeSz(2), []);%yang
        ...
        if this.arg.removeOS
            if this.NCol<this.NCol_protocol*2
                keepOS       = [1:this.NCol_protocol/2, 1+this.NCol_protocol*3/2:this.NCol_protocol*2];
            end
        end
        %---------------------------------------------------------------------
        ...
        for k = 1:kMax
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                if this.arg.removeOS
                    if this.NCol<this.NCol_protocol*2
                        nn=this.NCol_protocol*2-this.NCol;
                        block=padarray(block, nn,'pre');
                    end
                end
                %---------------------------------------------------------------------                
